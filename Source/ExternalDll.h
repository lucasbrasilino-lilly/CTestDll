...
/// SVObserver need to now what kind of images can be send to the DLL.
/// 
/// SVObserver will allocate an array of NULL HBITMAP handles and pass a pointer to this array to the function. 
/// The DLL will allocate the array, the _bstr_t in the structure will delete themselfs,
/// SVObserver is responsible for destroying the InputImageInformationStruct array.
/// \param[out] plArraySize SVO will send the reference to an uninitialized long. Set this to the number of images used.
/// \param[out] ppInputImageInformation SVO will send the reference to an uninitialized InputImageInformationStruct*.  
...
/// Deletes the array allocated in SVGetInputImageInformation
...
/// Destroys the arrays allocated in SVGetInputValueDefinitions
TOOLDLL_API HRESULT __stdcall SVDestroyInputValueDefinitionStructuresEx(InputValueDefinitionStructEx* paStructs);

/// Destroy the array defined in GetResultTableDefinitions
...
/// get the maximum row size for all Tables.
/// \param[in] lArraySize Array size
/// \param[out] pRowSizes preallocated array with the row sizes. The DLL may not delete, destroy, re-allocate this one. 

/* If this function is present  it will be called with each Reset.
This function will overwrite the values set in GetResultTableDefinitions or GetResultTableDefinitionsEx .
The function GetResultTablesMaxRowSize ist optional und should not be implementet if the external DLL will work with fixed Rowsizes.

if getResultTablesMaxRowSize is not defined, the length will be set using the values in ResultTableDefinitionStructEx.
*/
TOOLDLL_API HRESULT __stdcall GetResultTablesMaxRowSize(GUID tool, long lArraySize, int pRowSizes[]);

/// \param[in] size of the pResultTable array
/// \param[out] pResultTables preallocated VARIANT Array to copy the table variants to.
/// the variants are cleared in SVObserver
TOOLDLL_API HRESULT __stdcall GetResultTables(GUID tool, long  lArraySize, VARIANT* pResultTables);

/// Get the maximum Array sizes for Result Value arrays. 
/// SVObserver will expect arrays not larger than this during run
/// \param[in] size The size of the array pArraySizes
/// \param[in\out] pArraySizes preallocated by SVObserver
TOOLDLL_API HRESULT __stdcall GetResultValuesMaxArraySize(GUID tool, long size, int pArraySizes[]);

/// Get the definitions for Result Tables. 
/// called In SVO 8.20
/// 
/// \param[out] pSize SVO gets the number of Result Tables used
/// \param[out] ppaResultTableDefs an array of size pSize of table definitions. The DLL allocates this array.
/// This array is destroyed in the function DestroyResultTableDefinitionStructures 
TOOLDLL_API HRESULT __stdcall GetResultTableDefinitions(long* pArraySize, ResultTableDefinitionStruct** ppaResultTableDefs);


/// From SVObserver 10 on, this function is used to get the result table definitions
/// This array is destroy in the function DestroyResultTableDefinitionStructuresEx 
